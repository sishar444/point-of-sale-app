...
products = [
    ...
    {"id":21, "name": "Professor Rossetti's Bananas", "department": "fruit", "aisle": "fruits", "price": 0.79, "price_per": "pound"}
...
def sum_cart_subtotal(cart):
    total = 0
    for item in cart:
        if item["product"]["price_per"] == "item":
            total = total + item["product"]["price"]
        else:
            total_per_pound = item["pounds"] * item["product"]["price"]
            total = total + total_per_pound
...
def print_cart_items(cart):
    ...
    for item in cart:
        if item["product"]["price_per"] == "item":
            shopping_cart_string = shopping_cart_string + " + " + item["product"]["name"] + " (${0:.2f})\n".format(item["product"]["price"])
        else:
            shopping_cart_string = shopping_cart_string + " + " + item["product"]["name"] + " (${0:.2f})\n".format(item["product"]["price"]*item["pounds"])
...
while True:
    ...
    if product_by_id(product_id):
        ...
        if product["price_per"] == "pound":
            ...
            cart_list.append({"product": product, "pounds": pounds})
        else:
            cart_list.append({"product": product, "pounds": None})
    else:
